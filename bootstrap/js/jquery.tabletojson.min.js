...
!(function(a) {
	...
	var b = function(b, c, d) {
		(this.$element = a(b)),
			...
			(this.options = a.extend({}, a.fn.tableToJSONCell.defaults, d)),
			this.init();
	};
	(b.prototype = {
		constructor: b,
		value: function(b) {
			...
			if (e) d = e;
				...
				if (c.extractor || c.textExtractor)
		...
		extractedValue: function() {
			...
			return (
				a.isFunction(b)
					? (c = b(this.index, this.$element))
					: 'object' == typeof b &&
					  a.isFunction(b[this.index]) &&
					  (c = b[this.index](this.index, this.$element)),
				'string' == typeof c ? a.trim(c) : c
			);
		},
		colSpan: function() {
			var a = 1;
			return (
				this.$element.attr('colSpan') &&
					(a = parseInt(this.$element.attr('colSpan'), 10)),
				a
			);
		},
		rowSpan: function(a) {
			return (
				1 === arguments.length
					? (this.cachedRowSpan = a)
					: this.cachedRowSpan ||
					  ((this.cachedRowSpan = 1),
					  this.$element.attr('rowSpan') &&
							(this.cachedRowSpan = parseInt(
								this.$element.attr('rowSpan'),
								10
							))),
				this.cachedRowSpan
		...
		withColSpan: function(a) {
			var b = a;
			if (this.$element.attr('colSpan')) {
				var c = this.colSpan();
				if (c > 1) {
					b = [];
					for (var d = 0; d < c; d++) b.push(a);
				}
			}
			return b;
		},
	...
	(function(a) {
		...
		var b = function(b, c) {
			(this.$element = a(b)),
				(this.cells = []),
				(this.options = a.extend({}, a.fn.tableToJSONRow.defaults, c)),
				this.init();
		};
		(b.prototype = {
			constructor: b,
			...
			values: function(b) {
				for (
					...
					if (
						((e = this.cells[g].value(c)),
						1 === this.cells[g].colSpan())
					)
						this.ignoreColumn(f) || (d = d.concat(e)), f++;
					else
						for (var h = 0; h < e.length; h++)
							this.ignoreColumn(f) || (d = d.concat(e[h])), f++;
				return d;
			},
			ignoreColumn: function(a) {
				return this.options.onlyColumns
					? this.options.onlyColumns.indexOf(a) < 0
					: this.options.ignoreColumns.indexOf(a) > -1;
			},
			init: function() {
				var b = this;
				this.$element
					.children(this.options.cellSelector)
					.each(function(c, d) {
	...
	(function(a) {
		...
		var b = function(b, c) {
			(this.$element = a(b)),
				(this.rows = []),
				(this.options = a.extend({}, a.fn.tableToJSON.defaults, c)),
				this.init();
		};
		(b.prototype = {
			constructor: b,
			headings: function() {
				return this.rows.length > 0 && !this.options.headings
					? this.rows[0].values({
							extractor: null,
							textExtractor: null
					  })
				...
				for (c; c < this.rows.length; c++)
					if (!this.ignoreRow(this.rows[c], c))
						if (this.options.includeRowId) {
							var d =
									'string' == typeof this.options.includeRowId
			...
			ignoreRow: function(a, b) {
				return (
					(this.options.ignoreRows &&
						this.options.ignoreRows.indexOf(b) > -1) ||
					(a.$element.data('ignore') &&
						'false' !== a.$element.data('ignore')) ||
					(this.options.ignoreHiddenRows &&
						!a.$element.is(':visible')) ||
					(this.options.ignoreEmptyRows && a.isEmpty())
				);
			},
			addRow: function(a, b) {
			...
			init: function() {
				var b = this,
					...
					d = null;
				this.$element
					.children(this.options.rowParentSelector)
					.children(this.options.rowSelector)
					.each(function(e, f) {
						(d = a(f).tableToJSONRow(b.options)),
							(c = b.addRow(d, c));
					}),
					a.proxy(function() {
						this.$element.triggerHandler('init', this);
